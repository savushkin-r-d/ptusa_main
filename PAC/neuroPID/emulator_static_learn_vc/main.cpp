...
int _tmain(int argc, _TCHAR* argv[])
    ...
    mlp *emulator = new mlp( 2 * INPUTS_COUNT + 1, 10, 1 );

    //2. Загружаем данные.
    stored_sample *s_data = new stored_sample();
    s_data->load_from_file( "..\\emul_learning_z.data" );

    s_data->create_learn_sample( 2 * INPUTS_COUNT + 1, 35, 1, 1 );
    s_data->save_to_file( "..\\aa.txt" );
 
    emulator->init_weights();
    emulator->static_learn( 0.00005f, s_data, 100000 );
    emulator->save_to_file( "..\\emul_k_z.data" );
    return 0;
    }
