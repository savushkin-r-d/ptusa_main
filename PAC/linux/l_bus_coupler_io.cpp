...
int io_manager_linux::net_init(io_node* node) {
  ...
  if (G_DEBUG) {
    printf(
        "io_manager_linux:net_init() : socket %d is successfully"
        " connected to \"%s\":\"%s\":%d\n",
...
int io_manager_linux::write_holding_registers(io_node* node,
...
#endif  // DEBUG_BK_MIN
          res = read_input_registers(nd, start_read_address + start_register,
                                     registers_count);
...
//-----------------------------------------------------------------------------
io_manager_linux::io_manager_linux() { writebuff = &buff[13]; }
//-----------------------------------------------------------------------------
