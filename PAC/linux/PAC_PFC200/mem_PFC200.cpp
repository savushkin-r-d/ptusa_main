...
//-----------------------------------------------------------------------------
eeprom_PFC200::~eeprom_PFC200() { nvram_close(); }
//-----------------------------------------------------------------------------
int eeprom_PFC200::read(void *buff, u_int count, u_int start_pos) {
  if (0 == NV_ram_data) return 0;

  memcpy(buff, NV_ram_data + get_available_start_pos() + start_pos, count);
...
int eeprom_PFC200::write(void *buff, u_int count, u_int start_pos) {
  ...
  memcpy(NV_ram_data + get_available_start_pos() + start_pos, buff, count);
