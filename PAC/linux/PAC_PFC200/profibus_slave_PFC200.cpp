...
int profibus_slave_PFC200::init() {
  ...
  tDeviceInfo astDeviceList[MAX_DEVICE_LIST_ENTRIES];  // The list of devices
                                                       // given by the ADI.
  ...
  if (DAL_SUCCESS == iDalResult) {
    ...
    if (DAL_SUCCESS == iDalResult) {
      // Get the device list.
      iDalResult = get_device_list(pstAdi, &nrDevicesFound, astDeviceList,
                                   MAX_DEVICE_LIST_ENTRIES);
    }

    if (DAL_SUCCESS == iDalResult) {
      // Get the PROFIBUS slave device ID.
      iDalResult = get_device_id(nrDevicesFound, astDeviceList, &iDpsDeviceId);
    }
...
//------------------------------------------------------------------------------
int32_t profibus_slave_PFC200::scan_for_devices(
...
int32_t profibus_slave_PFC200::get_device_id(size_t ulNrDevicesFound,
                                             tDeviceInfo* pastDeviceList,
                                             tDeviceId* piDpsDeviceId) {
  int32_t iDalResult = DAL_SUCCESS;
  uint_t uiIndex;
...
//------------------------------------------------------------------------------
int32_t profibus_slave_PFC200::setup_watchdog(
    tApplicationDeviceInterface* pstAdi) {
  int32_t iDalResult = DAL_FAILURE;
  if (NULL == pstAdi) return iDalResult;
...
int32_t profibus_slave_PFC200::configure_device(
  ...
  pbSlotCfgDesc_t astRealSlotCfgDesc[2];  // Real slot configuration
                                          // description.

  // Set up the real configuration.

  // Slot 1 - 16 byte DP-Master output (BYTE Array [output_byte_size]
  // (DPM-OUT)).
...
int profibus_slave_PFC200::eval() {
  ...
  if (iDalResult == DAL_SUCCESS) {
    ...
    if (iDalResult == DAL_SUCCESS) {
      iDalResult =
          pstAdi->ReadBytes(iDpsDeviceId, 0, 0, 244, &aucPlcPrcImgInp[0]);
  ...
  if (iDalResult == DAL_SUCCESS) {
    ...
    if (iDalResult == DAL_SUCCESS) {
      iDalResult =
          pstAdi->WriteBytes(iDpsDeviceId, 0, 0, 244, &aucPlcPrcImgOutp[0]);
  ...
  if (iDalResult == DAL_SUCCESS) {
    printf(
        "PROFIBUS slave DP PLC input data = "
...
int profibus_slave_PFC200::get_int(int byte_offset) {
  int res = 0;
  if (byte_offset < 242) {
    res = 256 * aucPlcPrcImgInp[byte_offset] + aucPlcPrcImgInp[byte_offset + 1];
  }
...
//------------------------------------------------------------------------------
void profibus_slave_PFC200::set_bool(int byte_offset, int bit_offset,
                                     bool val) {
  if (byte_offset < 244 && bit_offset < 8) {
...
//------------------------------------------------------------------------------
int32_t profibus_slave_PFC200::stop_watchdog(
    tApplicationDeviceInterface* pstAdi) {
  int32_t iDalResult = DAL_FAILURE;
  if (NULL == pstAdi) return iDalResult;
...
//------------------------------------------------------------------------------
int32_t profibus_slave_PFC200::close_device(
    tApplicationDeviceInterface* pstAdi) {
  int32_t iDalResult = DAL_FAILURE;
  if (NULL == pstAdi) return iDalResult;
...
//------------------------------------------------------------------------------
int32_t profibus_slave_PFC200::release_ADI(
    tApplicationDeviceInterface* pstAdi) {
  int32_t iDalResult = DAL_FAILURE;
  if (NULL == pstAdi) return iDalResult;
