...
extern int errno;
//-----------------------------------------------------------------------------
int io_manager_windows::net_init(io_node* node)
{
...
//-----------------------------------------------------------------------------
int io_manager_windows::write_outputs()
{
...
//-----------------------------------------------------------------------------
int io_manager_windows::e_communicate(io_node* node, int bytes_to_send, int bytes_to_receive)
{
...
int io_manager_windows::read_input_registers(io_node* node, unsigned int address, unsigned int quantity, unsigned char station /*= 0*/)
{
    buff[0] = 's';
    buff[1] = 's';
    buff[2] = 0;
...
int io_manager_windows::write_holding_registers(io_node* node, unsigned int address, unsigned int quantity, unsigned char station)
{
    unsigned int bytes_cnt = quantity * 2;
    buff[0] = 's';
    buff[1] = 's';
...
//-----------------------------------------------------------------------------
int io_manager_windows::read_inputs()
{
    if (0 == nodes_count) return 0;
...
//-----------------------------------------------------------------------------
void io_manager_windows::disconnect(io_node* node)
{
    if (node->sock)
    {
...
//-----------------------------------------------------------------------------
io_manager_windows::io_manager_windows()
{
    writebuff = &buff[13];
}
//-----------------------------------------------------------------------------
