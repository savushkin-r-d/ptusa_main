...
int tcp_communicator_win::evaluate()
    ...
    //------------------------------------------------------------------------------
    int tcp_communicator_win::sendall(int sockfd, unsigned char* buf, int len,
...
//------------------------------------------------------------------------------
int tcp_communicator_win::recvtimeout( u_int s, u_char *buf, int len, int timeout, int usec )
    {
    // Настраиваем  file descriptor set.
    fd_set fds;
    FD_ZERO( &fds );
