...
struct socket_state {
  ...
  int is_listener;  ///< Сокет является инициатором соединения ( = 0 )/сокет
                    ///< является слушателем ( != 0 ).
  int evaluated;  ///< В данном цикле уже произошел обмен информацией по данному
                  ///< сокету.
  int clID;  ///< Идентификатор клиента для идентификации того, не занят ли уже
             ///< сокет другим клиентом.
...
class tcp_communicator_win : public tcp_communicator {
 public:
  ...
  static int recvtimeout(u_int s, u_char *buf, int len, int timeout, int usec);
 ...
 private:
  ...
  u_long
      glob_last_transfer_time;  ///< Время последней успешной передачи данных.

  timeval tv;  ///< Задержка ожидания функции опроса состояний сокетов, 0 по
               ///< умолчанию.
...
/// @return -2   - ошибка таймаута.
/// @return >= 0 - размер реально считанных данных.
int recvtimeout(u_int s, u_char *buf, int len, int timeout, int usec);
}
;
