/*
Last Update at 2010/04/20 [yyyy/mm/dd] By Tim Tsai.
*/
#ifndef __MINIOS7__
#define __MINIOS7__

#define _uPAC_5000_ 1
...
/* Timer functions II */
void T2_UpdateCurrentTimeTicks(
    void); /* every loop must call T2_UpdateCurrentTimeTicks() to get new
              time.*/
...
(INPUT) cCmd: cmd for send to COM port(I-7000/I-87K).
              DON'T add '\r' at the end of cCmd, SendCmdTo7000() will add check
sum(if needed) & '\r' after cCmd . (INPUT) iChksum: 0: disable, 1: enable.
*/
...
int ReceiveResponseFrom7000_1(int iPort, unsigned char *cCmd, long lTimeout,
                              int iChksum);
/*
(INPUT)iPort:can be 1~8.
(INPUT)lTimeout: unit is ms. (*****)
...
void SetComPortBufferSize(int port, int in_size,
                          int out_size); /* [2006/07/12] */
...
/*
  ...
  IsCom(port)/ReadCom(port) just for backword compatible, it also will call
...
extern int (*InstallCom_[COM_PORT_NO + 1])(unsigned long baud, int data,
                                           int parity, int stop);
...
extern int (*SetDataFormat_[COM_PORT_NO + 1])(int databit, int parity,
                                              int stopbit);
extern void (*SetCtsControlMode_[COM_PORT_NO + 1])(
    int mode); /*[2006/01/19] add*/
extern void (*SetRtsControlMode_[COM_PORT_NO + 1])(
    int mode); /*[2006/01/19] add*/
extern void (*SetXonXoffControlMode_[COM_PORT_NO + 1])(
    int mode);                                              /*[2006/01/19] add*/
...
extern int (*SetComFifoTriggerLevel_[COM_PORT_NO + 1])(
    int level); /*[2006/06/29] */
extern void (*SetComPortBufferSize_[COM_PORT_NO + 1])(
    int in_size, int out_size); /* [2006/07/12] */
...
extern void (*InstallComInputData_[COM_PORT_NO + 1])(
    int (*)(unsigned char));                                /* [2007/10/02] */
...
/* [11/06/2003] add Software flow control(Xon/Xoff) for COM1~COM8 except COM2
        COM2 is RS-485, used in half-duplex mode, need not software flow
   control.
*/
...
/*
 ...
 Even after all InstallCom_1() also can use these 3 functions to send message to
 STDIO COM port.
*/
...
/* 2004/02/26 add function usr burst mode to read date/time from RTC
 * chip(DS-1302) */
typedef struct {
  int year;
...
/*
 ...
 Even after all InstallCom_2() also can use these 3 functions to send message to
 COM2.
*/
...
/*
The delay functions(Dealy()/Delay_1()/Delay_2() by default using system's timer
0. user can call SetDelayTimer(1); set it to use timer 1.
...
/*
The user timer
functions(InstallUserTimerFunction_us()/InstallUserTimerFunction_ms() by default
using system's timer 0. user can call SetUserTimer(1); set it to use timer 1.
...
If the program does not using INT 9, set Int9Flag to 0 will get better
performance.

Int1cFlag : default value is 1;
When Int1cFlag != 0, the new timer ISR will call INT 0x1c every 55 ms.
If the program does not using INT 0x1C, set Int1cFlag to 0 will get better
performance.
...
void GPRS_SetPower(int mode);
void GPRS_Reset(void);

void GPS_SetPower(int mode);
void GPS_Reset(void);

/*
[2009/08/12] add the PIO pin definition
*/
#define PIN_DIO_4 4
#define PIN_DIO_9 12
#define PIN_DIO_14 13
#define PIN_TO_0 15
#define PIN_TI_0 11
#define PIN_TO_1 1
#define PIN_TI_1 0
...
/*
[2010/03/03] Add functions for 512K BBSRAM
*/
#define OffsetError -100
#define S256_BlockSize 256
#define S256_BlockWordSize 128
extern unsigned S256_MaxBlock;
//	S256_MaxBlock=2048 	 for 512K BBSRA/NVSRAM
//  S256_MaxBlock=4096	 for 1M NVSRAM
/*---------------------------------------------------------------------*/
int S256_Init(void);
/*
Before use S256/S512 must call this function.
[return value]:
 512: ram size is 512K (512K BBSRAM or 512K NVSRAM).
 1024: ram size is 1024K (1024K NVSRAM).
   0: can not find BBSRAM or NVSRAM.
*/
...
[input]
address:0-0x7FFFF for 512K BBSRAM/NVSRAM, 0-0xFFFFF for 1024K NVSRAM.
...
[return value]:
return the readed data number.
On success S256_WriteFn returns the number of data actually written.
if the address is out of range S256_WriteFn return 0.
*/
/*---------------------------------------------------------------------*/
...
/*
[2010/03/10] add functions for both BBSRAM and NVSRAM
If you like the block/addr mode, just use the functions S256_xxxx().
If you like the linear address mode, just use the functions nvSRAM_xxxx().
*/
unsigned nvSRAM_Init(void);
/*
return value: the RAM size in kilobytes.
*/

int nvSRAM_Read(unsigned long addrL, void *ptr, int bytcnt);
int nvSRAM_Write(unsigned long addrL, void *ptr, int bytcnt);
...
/*[2010/03/22] add function for rotary switch*/
int RotarySwitch_Read(void);
/*
return value: 0~9.
*/

/*[2010/03/26] add function for Battery status*/
int BatteryStatus_Read(void);
/*
return value: bit0 & bit1, 1:status OK. 0: error.
*/
