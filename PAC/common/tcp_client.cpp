...
tcp_client::tcp_client(const char* client_ip, unsigned int client_port,
                       unsigned int client_id, unsigned char alarm_subclass,
...
tcp_client* tcp_client::Create(const char* ip, unsigned int port,
                               unsigned int id, unsigned char alarm_subclass,
                               unsigned int exchange_buf_size /*= 256*/,
                               unsigned long send_receive_timeout /*= 100*/) {
#if defined WIN_OS
  return new win_tcp_client(ip, port, id, alarm_subclass, exchange_buf_size,
                            send_receive_timeout);

#elif defined LINUX_OS
  return new linux_tcp_client(ip, port, id, alarm_subclass, exchange_buf_size,
                              send_receive_timeout);
...
int tcp_client::Communicate(unsigned int bytestosend) { return 0; }

int tcp_client::get_id() { return id; }
  ...
  if (connectedstate != ACS_CONNECTED) {
    if (get_delta_millisec(async_last_connect_try) > reconnectTimeout ||
      ...
      if (connectres == ACS_DISCONNECTED) {
        ...
        reconnectTimeout > maxreconnectTimeout ? maxreconnectTimeout
                                               : reconnectTimeout *= 2;
        return 0;
      }
...
int tcp_client::AsyncConnect() { return 0; }
...
int tcp_client::get_async_result() { return async_result; }

int tcp_client::get_connected_state() { return connectedstate; }

int tcp_client::get_socket() { return socket_number; }
