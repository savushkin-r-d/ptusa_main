...
enum workParameters {
  ...
  PV_RET_DEL,  // через сколько литров после начала подачи V2 включать
               // возвратный насос
  ...
  P_V_SANITIZER_RINSING,  // Объем ополаскивания после мойки дезинфицирующим
                          // средством
  P_TM_MAX_TIME_OPORBACHOK,  // Максимальное время от пропадания нижнего уровня
                             // в бачке до завершении операции опорожнения бачка
...
enum SELFCLEAN_PARAMS {
  ...
  SCP_LITERS_AFTER_LL_TS,  // Кол-во л для откачивания после исчезновения НУ в
                           // ТЩ
  SCP_LITERS_AFTER_LL_TK,  // Кол-во л для откачивания после исчезновения НУ в
                           // ТК
  SCP_LITERS_AFTER_LL_TW,  // Кол-во л для откачивания после исчезновения НУ в
                           // ТВ
...
class cipline_tech_object : public tech_object {
 ...
 public:
  ...
  char circ_water_no_pump_stop;  // не останавливать насос при поялении верхнего
                                 // уровня в бачке
  char circ_medium_no_pump_stop;  // не останавливать насос при поялении
                                  // верхнего уровня в бачке на щелочи и кислоте
