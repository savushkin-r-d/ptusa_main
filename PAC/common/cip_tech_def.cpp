  ...
  switch (ncmd) {
    case MCMD_NEXT:
      if (state != 0 && curstep != 555) {
        rt_par_float[STP_STEPS_OVER] = rt_par_float[STP_STEPS_OVER] +
                                       1;  // количество пропущенных операций
    ...
    case MCMD_EVALUATE:
      ...
      if (state == 0 &&
        ...
        if (scenabled) {
          if (scline > 0 &&
              scline != nmr)  // если идет самоочистка танков, то нельзя
                              // включить мойку на других линиях.
          {
            return 0;
          }
          if (0 == scline &&
  ...
  if (0 == first_init_flag) {
    ...
    switch (no_liquid_phase) {
      case 0:  // начало первого цикла добавления раствора - запоминаем время
        ...
        no_liquid_is_warning = 0;
        break;
      case 1:  // не первый цикл. сравниваем время с предыдущим. Если совпадают
               // с допуском до 10% выдаем 1 раз ошибку.
