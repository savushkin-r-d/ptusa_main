...
cip_object_stats* cip_stats::stats_if_exists(char* objid,
                                             cip_object_stats* cip_if_empty) {
  std::map<char*, cip_object_stats*>::iterator ret = obj_stats.find(objid);
  if (ret == obj_stats.end()) {
    return cip_if_empty;
  } else {
