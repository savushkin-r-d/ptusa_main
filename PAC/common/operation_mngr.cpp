        ...
        if ( res )
            ...
            switch ( current_state )
                ...
                case state_idx::RUN:
                    ...
                    unit->set_mode( num_operation, state_idx::IDLE );
                    unit->set_err_msg( "автоотключение по запросу",
                        num_operation, 0, tech_object::ERR_MSG_TYPES::ERR_DURING_WORK );
                    break;

                case state_idx::STARTING:
                    unit->set_mode( num_operation, state_idx::RUN );
                    break;

                case state_idx::PAUSING:
                    unit->set_mode( num_operation, state_idx::PAUSE );
                    break;
                ...
                case state_idx::STOPPING:
                    unit->set_mode( num_operation, state_idx::STOP );
                    break;                    
...
int operation::process_auto_switch_on()
    ...
    if ( is_first_goto_next_state )
        {
        auto result = unit->set_mode( num_operation, operation::RUN );
        if ( result == 0 )
            {
            unit->set_err_msg( "автовключение по запросу", num_operation, 0, WARN );
            return 0;
            }
        else
            {
            unit->set_err_msg( "нет автовключения по запросу", num_operation, 0, ERR );
        ...
        if ( unit->check_operation_on( num_operation, false ) == 0 )
            {
            unit->set_err_msg( "автовключение по запросу", num_operation, 0, WARN );
            unit->set_mode( num_operation, operation::RUN );
            return 0;
            }
        ...
        if ( get_delta_millisec( start_warn ) > dt )
            {
            unit->check_operation_on( num_operation );
            unit->set_err_msg( "нет автовключения по запросу", num_operation, 0, ERR );
            start_warn = get_millisec();
            }
        ...
        if ( get_delta_millisec( start_wait ) > wt )
            {
            unit->set_err_msg( "автовключение по запросу отключено", num_operation, 0, ERR );
            was_fail = true;
            }
        }
