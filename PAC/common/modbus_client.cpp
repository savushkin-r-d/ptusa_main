...
modbus_client::modbus_client(unsigned int id, char* ip, unsigned int port,
...
modbus_client::~modbus_client() { delete tcpclient; }

int modbus_client::read_discrete_inputs(unsigned int start_address,
  ...
  if (res != 9 + quantity / 8 + (quantity % 8 ? 1 : 0)) {
    if (G_DEBUG) {
      printf(
          "modbus_client_%d: read_discrete_inputs - error in exchange. "
          "Received %u bytes\n",
...
int modbus_client::read_coils(unsigned int start_address,
                              unsigned int quantity) {
  tcpclient->buff[0] = 0;
  tcpclient->buff[1] = 0;
  tcpclient->buff[2] = 0;
  ...
  if (res != 9 + quantity / 8 + (quantity % 8 ? 1 : 0)) {
    if (G_DEBUG) {
      printf(
          "modbus_client_%d: read_coils - error in exchange. Received %u "
          "bytes\n",
          tcpclient->get_id(), res);
    }
    return 0;
...
int modbus_client::read_holding_registers(unsigned int address,
                                          unsigned int quantity) {
  tcpclient->buff[0] = 0;
  tcpclient->buff[1] = 0;
  tcpclient->buff[2] = 0;
  ...
  if (res != 9 + quantity * 2) {
    if (G_DEBUG) {
      printf(
          "modbus_client_%d: read_holding_registers - error in exchange. "
          "Received %u bytes\n",
          tcpclient->get_id(), res);
    }
    return 0;
...
int modbus_client::read_input_registers(unsigned int address,
                                        unsigned int quantity) {
  tcpclient->buff[0] = 0;
  tcpclient->buff[1] = 0;
  tcpclient->buff[2] = 0;
  ...
  if (res != 9 + quantity * 2) {
    if (G_DEBUG) {
      printf(
          "modbus_client_%d: read_input_registers at %u- error in exchange. "
          "Received %u bytes\n",
...
int modbus_client::write_coil(unsigned int address, unsigned char value) {
  ...
  if (res != 12) {
    if (G_DEBUG) {
      printf(
          "modbus_client_%d: write coil %u - error in exchange. Received %d "
          "bytes\n",
          tcpclient->get_id(), address, res);
    }
    return 0;
...
int modbus_client::force_multiply_coils(unsigned int address,
                                        unsigned int quantity) {
  unsigned char bytecount =
      (unsigned char)quantity / 8 + (((unsigned char)quantity % 8) ? 1 : 0);
  tcpclient->buff[0] = 0;
  tcpclient->buff[1] = 0;
  tcpclient->buff[2] = 0;
  tcpclient->buff[3] = 0;
  ...
  if (res != 12) {
    if (G_DEBUG) {
      printf(
          "modbus_client_%d: force_multiply_coils at %u- error in exchange. "
          "Received %d bytes\n",
          tcpclient->get_id(), address, res);
    }
    return 0;
...
int modbus_client::write_multiply_registers(unsigned int address,
                                            unsigned int quantity) {
  ...
  if (res != 12) {
    if (G_DEBUG) {
      printf(
          "modbus_client_%d: force_multiply_registers at %u- error in "
          "exchange. Received %d bytes\n",
          tcpclient->get_id(), address, res);
    }
    return 0;
  }
...
void modbus_client::set_byte(int address, unsigned char value) {
  if (address > (int)tcpclient->buff_size - write_buff_start -
                    (int)sizeof(unsigned char) ||
      address + write_buff_start < 0) {
    return;
  }
...
unsigned char modbus_client::get_byte(int address) {
  if (address > (int)tcpclient->buff_size - read_buff_start -
                    (int)sizeof(unsigned char) ||
      address + read_buff_start < 0) {
    return 0;
  }
...
void modbus_client::set_bit(unsigned int address, int value) {
  ...
  tcpclient->buff[write_buff_start + numbyte] ^=
      (-(value ? 1 : 0) ^ tcpclient->buff[write_buff_start + numbyte]) &
      (1 << numbit);
}
...
void modbus_client::reg_set_bit(unsigned int reg, unsigned int offset,
                                int value) {
...
int modbus_client::get_id() { return tcpclient->get_id(); }

int modbus_client::async_read_discrete_inputs(unsigned int start_address,
                                              unsigned int quantity) {
...
int modbus_client::get_async_result() { return tcpclient->get_async_result(); }

int modbus_client::get_connected_state() {
  return tcpclient->get_connected_state();
}

int modbus_client::async_read_coils(unsigned int start_address,
                                    unsigned int quantity) {
  int ar = get_async_result();
  if (ar == tcp_client::AR_BUSY) {
    return 0;
...
int modbus_client::async_read_holding_registers(unsigned int address,
                                                unsigned int quantity) {
  int ar = get_async_result();
  if (ar == tcp_client::AR_BUSY) {
    return 0;
...
int modbus_client::async_read_input_registers(unsigned int address,
                                              unsigned int quantity) {
  int ar = get_async_result();
  if (ar == tcp_client::AR_BUSY) {
    return 0;
...
int modbus_client::async_force_multiply_coils(unsigned int address,
                                              unsigned int quantity) {
  int ar = get_async_result();
  if (ar == tcp_client::AR_BUSY) {
    return 0;
...
int modbus_client::async_write_multiply_registers(unsigned int address,
                                                  unsigned int quantity) {
  int ar = get_async_result();
  if (ar == tcp_client::AR_BUSY) {
    return 0;
...
int modbus_client::async_read_write_multiply_registers(
    unsigned int readaddress, unsigned int readquantity,
    unsigned int writeaddress, unsigned int writequantity) {
  int ar = get_async_result();
  if (ar == tcp_client::AR_BUSY) {
    return 0;
  } else {
