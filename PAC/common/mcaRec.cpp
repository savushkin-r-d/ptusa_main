...
TRecipeManager::TRecipeManager(int lineNo)
    : lineNo(lineNo),
      currentRecipe(0),
  ...
  recipeList =
      new char[(recipeNameLength * UNICODE_MULTIPLIER + 12) * recipePerLine];
  strcpy(recipeList, "");
...
unsigned long TRecipeManager::startAddr() { return startAddr(currentRecipe); }
...
float TRecipeManager::getRecipeValue(int recNo, int valueNo) {
  unsigned long temp;
  ReadMem(startAddr(recNo) + startRecipeParamsOffset + 4 * valueNo, 4,
          (unsigned char*)&temp);
  if (temp != 0xFFFFFFFF) {
    return *((float*)&temp);
  } else {
...
int TRecipeManager::setRecipeValue(int recNo, int valueNo, float newValue) {
  recipechanged = 1;
  recipechangechecktime = get_millisec();
  WriteMem(startAddr(recNo) + startRecipeParamsOffset + 4 * valueNo, 4,
...
int TRecipeManager::LoadRecipeToParams(int recipeNo, int recipeStartPos,
                                       int paramsStartPos, int parQuantity,
                                       run_time_params_float* par) {
  if (recipeNo < recipePerLine &&
      recipeStartPos + parQuantity < GetParamsCount() &&
      (unsigned int)paramsStartPos + (unsigned int)parQuantity <
          par->get_count()) {
    for (int i = 0; i < parQuantity; i++) {
      par[0][paramsStartPos + i] = getRecipeValue(recipeNo, recipeStartPos + i);
#ifdef DEBUG
      printf("\n\rsetparamm %d %f\n\r", paramsStartPos + i,
             par[0][paramsStartPos + i]);
#endif
    }
#ifdef DEBUG
    printf("\n\rLoaded recipe %d\n\r", recipeNo);
...
int TRecipeManager::getCurrentRecipe() { return currentRecipe; }
  ...
  for (int i = 0; i < recipePerLine; i++) {
    if (getRecipeValue(i, RV_IS_USED) != 0) {
      ReadMem(startAddr(i), recipeNameLength, (unsigned char*)tmprecipename,
              true);
      sprintf(recipeList + strlen(recipeList), "%d##%s||", i + 1,
              tmprecipename);
    }
  }
}
...
#ifdef DEBUG
          printf(
              "\n\rOn recipe devices. Valve %u from param %d not found in "
              "device list\n\r",
              devNo, i);
#endif
          errflag = 1;
...
#ifdef DEBUG
          printf(
              "\n\rOff recipe devices. Valve %u from param %d not found in "
              "device list\n\r",
              devNo, i);
#endif
          errflag = 1;
...
int TMediumRecipeManager::setRecipeValue(int recNo, int valueNo,
                                         float newValue) {
  recipechanged = 1;
  recipechangechecktime = get_millisec();
  WriteMem(startAddr(recNo) + startRecipeParamsOffset + 4 * valueNo, 4,
...
int TMediumRecipeManager::LoadRecipeToParams(int recipeNo,
                                             saved_params<float, true> par) {
  switch (mediumType) {
    case MT_CAUSTIC:
...
int TMediumRecipeManager::getCurrentRecipe() { return currentRecipe; }
