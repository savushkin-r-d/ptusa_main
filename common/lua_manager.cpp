...
// Для каждого файла проверяется его наличие и версия (первая строка файла
// должна быть такой: "--version = 1"). Если файла нет или он имеет не
// поддерживаемую версию, то выдается соответствующее сообщение и работа
// приложения завершается. II Экспортируем в Lua классы и функции из C++.
