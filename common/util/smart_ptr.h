...
class smart_ptr {
 ...
 public:
  ...
  inline bool is_null() { return pointer == NULL; }

  inline bool is_null() const { return pointer == NULL; }

 private:
  /// @brief Закрытый оператор индексирования - запрет использования.
  type* operator[](unsigned int index) { return pointer; }

  /// @brief Закрытый оператор присваивания - запрет использования.
  smart_ptr<type>& operator=(const smart_ptr<type>& new_t) { return *this; }
};
//-----------------------------------------------------------------------------
/// @brief Защищенная работа с указателем.
///
/// Автоматическое освобождение памяти при уничтожении, переприсваивании
/// объекта.
template <class type>
class auto_smart_ptr : public smart_ptr<type> {
 public:
  /// @brief Конструктор по умолчанию.
  ...
  virtual ~auto_smart_ptr() { free(); }

  void replace_without_free(type* new_t) { smart_ptr<type>::pointer = new_t; }
