...
Подробную информацию по можно получить [здесь](https://www.lua.org).
...
Подробную информацию по можно получить [здесь](https://web.tecgraf.puc-rio.br/~celes/tolua).
...
Подробную информацию по можно получить [здесь](https://zlib.net/apps.html).
...
1. Скопируйте исходный файл и заголовочный файл в свой проект и включите subhook.c в свою сборку.
2. Только в Windows: определите SUBHOOK_STATIC перед включением subhook.h.

С CMake:

Скопируйте репозиторий subhook в дерево вашего проекта.

Вызовите add_subdirectory(path/to/subhook) в вашем CMakeLists.txt .

Необязательно: настройте способ создания библиотеки, установив эти переменные перед add_subdirectory(...):

SUBHOOK_STATIC - Сборка как статическая библиотека (по умолчанию отключена)
SUBHOOK_INSTALL - включить установку и упаковку целевых объектов/файлов с помощью CPack (по умолчанию отключено)
SUBHOOK_TESTS - Включить тесты (включено по умолчанию)
SUBHOOK_FORCE_32 BIT - Настройка для компиляции 32-разрядных двоичных файлов в 64-разрядных системах (по умолчанию отключено).
Использование CMake не является обязательным, библиотека может быть собрана без него (дополнительная конфигурация сборки не требуется).

Подробную информацию по можно получить [здесь](https://github.com/Zeex/subhook).
...
Подробную информацию по можно получить [здесь](https://github.com/google/googletest).
