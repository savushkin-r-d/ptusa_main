...
+ [3. Как собрать проект](#3-как-собрать-проект)
+ [4. Загрузка набора инструментов PLCnext Technology C++](#4-загрузка-набора-инструментов-plcnext-technology-c++)
+ [5. Как запустить проект](#5-как-запустить-проект)
+ [6. Используемые библиотеки](#6-используемые-библиотеки)
...
2. 1.  Общая схема работы программы контроллера (в [Drakon](https://drakonhub.com) diagram language):
...
2.3 При включении/выключении операции происходит следующее:

![Clone repository](../readme_images/tech_object__set_mode.png)

2.4 При выполнении команды происходит следующее:
...
## 4. Загрузка набора инструментов PLC next Technology C++ ##
...
### 4.2 Создание нового проекта на C++ в Visual Studio ##
...
5.1. После сборки и развертывания *`windows-default`* конфигурации cmake:
...
5.2. Запуск проекта в Microsoft Visual Studio Community 2022: `Выбрать элемент запуска -> ptusa_main.exe`.
