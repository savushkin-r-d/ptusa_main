...
## Реализация ##
Файлы с реализацией ```analog_emulator.h``` и ```analog_emulator.cpp``` находятся в папке ```ptusa_main/Pac/common```. В классе ```analog_emulator``` определены методы ```get_st_deviation()```, ```get_m_expec()```, ```get_min()```, ```get_max()``` возвращают значения стандартного отклонения, математического ожидания, минимального и максимального из возможных сгенерированных чисел.
