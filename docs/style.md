...
+ [Общие требования](#Общие-требования)
  ...
  + [Форматирование текста программы](#Форматирование-текста-программы)
  + [Написание комментария](#Написание-комментария)

## Общие требования

### Именование переменных

#### 1. Имена переменных составляются из слов английского языка

Транслитерация русских слов не допускается.

#### 2. Имена неконстантных переменных, классов, методов, структур состоят из строчных символов

Допускается использовать отдельные слова из прописных символов, слова разделятся символом подчеркивания «_».

#### 3. Имена констант, перечислений, элементов перечислений состоят только из прописных символов

Для разделения слов используется символ подчеркивания «_».

#### 4. Глобальные переменные начинаются с префикса «g_»

Для глобальных констант используется «G_». 

### Форматирование текста программы

#### 1. Длина строки равна 80 символам

Если строка программного кода не помещается целиком, она разбивается на несколько строк.
...
#### 1. Комментарии должны быть написаны предложениями на русском языке

Они должны быть хорошо составлены и иметь правильную пунктуацию, по возможности без сокращений.
...
#### 3. Комментарий пишется в стиле системы документирования **Doxygen**

Подробную информацию можно найти здесь - [Doxygen](https://www.doxygen.nl/).

#### 4. В начале каждого заголовочного модуля (*.h) следует служебная информация

В ней содержатся сведения об авторе (основной разработчик, занимающийся поддержкой данного модуля), назначении файла, директивах препроцессора (может быть опущено при отсутствии таковых).
...
#### 5. Каждое определение функции (объявление класса, структуры) отделяется специальным комментарием

Длина комментария - 80 символов. Это подобно заголовкам разделов. Вид комментария:
...
#### 14. Комментируется завершающая часть директив препроцессора 

Так как директива препроцессора #ifdef почти всегда расположена на некотором расстоянии от #endif, то всегда комментируется #endif. Пример:
