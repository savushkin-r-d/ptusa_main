...
/*
   ...
   errfunc) LUALIB_API int luaL_loadfile (lua_State *L, const char *filename)
...
TEST_F(LuaManagerTest, init_success) {
  ...
  mock_tech_object_manager *tech_mock =
      new mock_tech_object_manager();  // require par_mock
  ...
  EXPECT_CALL(*prj_mock, lua_load_configuration()).WillOnce(Return(0));

  EXPECT_CALL(*tech_mock, init_objects()).WillOnce(Return(0));
...
TEST_F(LuaManagerTest, init_creating_lua_context_failure) {
  ...
  subhook_t hook_lua_create_context =
      subhook_new((void *)luaL_newstate, (void *)mock_luaL_newstate_failure,
...
TEST_F(LuaManagerTest, init_check_file_failure) {
  subhook_t hook_check_file =
      subhook_new((void *)check_file, (void *)mock_check_file_failure,
                  SUBHOOK_64BIT_OFFSET);
...
TEST_F(LuaManagerTest, init_lua_load_configuration_failure) {
  ...
  EXPECT_CALL(*prj_mock, lua_load_configuration()).WillOnce(Return(42));
...
TEST_F(LuaManagerTest, init_luaL_loadfile_failure) {
  subhook_t hook_luaL_loadfile =
      subhook_new((void *)luaL_loadfile, (void *)mock_luaL_loadfile_failure_2,
                  SUBHOOK_64BIT_OFFSET);
  subhook_install(hook_luaL_loadfile);
  mock_project_manager *prj_mock = new mock_project_manager();
  mock_params_manager *par_mock = new mock_params_manager();
...
TEST_F(LuaManagerTest, init_lua_pcall_failure) {
  subhook_t hook_lua_pcall = subhook_new(
      (void *)lua_pcall, (void *)mock_lua_pcall_failure, SUBHOOK_64BIT_OFFSET);
...
TEST_F(LuaManagerTest, init_init_objects_failure) {
  ...
  EXPECT_CALL(*tech_mock, init_objects()).WillOnce(Return(42));
...
TEST_F(LuaManagerTest, void_exec_lua_method_success) {
  EXPECT_EQ(0, G_LUA_MANAGER->void_exec_lua_method("test_lua_object_str",
                                                   "test_lua_func_str",
                                                   "test_lua_c_func_str"));
}
...
TEST_F(LuaManagerTest, void_exec_lua_method_failure) {
  ...
  EXPECT_EQ(1, G_LUA_MANAGER->void_exec_lua_method("test_lua_object_str",
                                                   "test_lua_func_str",
                                                   "test_lua_c_func_str"));
...
/*
    TEST METHOD DEFENITION:
        ...
        LUA_API void lua_remove (lua_State *L, int idx)
        LUA_API int lua_pcall (lua_State *L, int nargs, int nresults, int
   errfunc) TOLUA_API const char* tolua_tostring (lua_State* L, int narg, const
   char* def)
*/
...
TEST_F(LuaManagerTest, char_no_param_exec_lua_method_success) {
  EXPECT_STREQ("Test passed", G_LUA_MANAGER->char_no_param_exec_lua_method(
                                  "test_lua_object_str", "test_lua_func_str",
                                  "test_lua_c_func_str"));
}
...
/*
    TEST METHOD DEFENITION:
        const char* char_exec_lua_method(const char *object_name,
                const char *function_name, int param, const char
   *c_function_name) const;
        ...
        LUA_API void lua_remove (lua_State *L, int idx)
        LUA_API int lua_pcall (lua_State *L, int nargs, int nresults, int
   errfunc) LUA_API void lua_pushnumber (lua_State *L, lua_Number n) TOLUA_API
...
TEST_F(LuaManagerTest, char_exec_lua_method_failure) {
  ...
  EXPECT_STREQ((char *)0, G_LUA_MANAGER->char_exec_lua_method(
                              "test_lua_object_str", "test_lua_func_str", 42,
                              "test_lua_c_func_str"));
...
TEST_F(LuaManagerTest, int_exec_lua_method_success) {
  EXPECT_EQ(0, G_LUA_MANAGER->int_exec_lua_method("test_lua_object_str",
                                                  "test_lua_func_str", 42,
                                                  "test_lua_c_func_str"));
}
...
TEST_F(LuaManagerTest, int_exec_lua_method_failure) {
  ...
  EXPECT_EQ(-1, G_LUA_MANAGER->int_exec_lua_method("test_lua_object_str",
                                                   "test_lua_func_str", 42,
                                                   "test_lua_c_func_str"));
...
TEST_F(LuaManagerTest, int_2_exec_lua_method_success) {
  EXPECT_EQ(0, G_LUA_MANAGER->int_2_exec_lua_method("test_lua_object_str",
                                                    "test_lua_func_str", 0, 42,
                                                    "test_lua_c_func_str"));
}
...
TEST_F(LuaManagerTest, int_2_exec_lua_method_failure) {
  ...
  EXPECT_EQ(-1, G_LUA_MANAGER->int_2_exec_lua_method("test_lua_object_str",
                                                     "test_lua_func_str", 0, 42,
                                                     "test_lua_c_func_str"));
...
TEST_F(LuaManagerTest, user_object_exec_lua_method_success) {
  EXPECT_STREQ("Test passed",
               (char *)(G_LUA_MANAGER->user_object_exec_lua_method(
                   "test_lua_object_str", "test_lua_func_str", 42,
                   "test_lua_c_func_str")));
}
...
/*
    TEST METHOD DEFENITION:
        int lua_manager::exec_Lua_str( const char *Lua_str, const char
   *error_str, bool is_print_error_msg = true ) const
...
TEST_F(LuaManagerTest, reload_script_lua_state_null_failure) {
  ...
  // test_reload_script
  char ans[100];
  EXPECT_EQ(1, G_LUA_MANAGER->reload_script(INT_MAX, "test_lua_func_str", ans,
                                            sizeof(ans)));
...
TEST_F(LuaManagerTest, reload_script_check_file_version_failure) {
  subhook_t hook_check_file =
      subhook_new((void *)check_file, (void *)mock_check_file_failure_2,
                  SUBHOOK_64BIT_OFFSET);
  subhook_install(hook_check_file);
...
TEST(lua_manager, error_trace) {
  ...
  EXPECT_EQ(0,
            luaL_dostring(L, "t.no_exist3 = function(a, b) return a + b end"));

  EXPECT_NE(
      0, G_LUA_MANAGER->void_exec_lua_method("t", "no_exist1", "error_trace"));
  EXPECT_NE(
      0, G_LUA_MANAGER->void_exec_lua_method("t", "no_exist2", "error_trace"));
  EXPECT_NE(
      0, G_LUA_MANAGER->void_exec_lua_method("t", "no_exist3", "error_trace"));
