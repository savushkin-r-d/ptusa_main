...
system =
  ...
  // Should fail without correct data - no init_devices_properties()
  // Lua-function.
...
Resetting params (command line parameter "rcrc").
  ...
  std::array<const char*, 12> argv_path{
      "ptusa_main.exe", "--port",          "20000", "--sys_path",
      "./sys/",         "--path",          "./",    "--extra_paths",
      "./dairy_sys/",   "--sleep_time_ms", "5",     "main.plua"};
