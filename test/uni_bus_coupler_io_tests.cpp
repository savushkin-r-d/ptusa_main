...
int __stdcall fail_socket(int, int, int) { return -1; }

int __stdcall fail_setsockopt(int, int, int, const char*, int) { return 1; }

#ifdef WIN_OS
int __stdcall fail_ioctlsocket(int, long, long) { return 1; }
#else
int fail_fcntl(int, int) { return 1; }
#endif
...
int __stdcall fail_getsockopt(int, int, int, char*, int*) { return -1; }

int __stdcall success_getsockopt(int, int, int, char*, int*) { return 0; }
...
TEST(uni_io_manager, net_init) {
  ...
  io_manager::io_node node(io_manager::io_node::TYPES::PHOENIX_BK_ETH, 1000,
                           "127.0.0.1", "A100", 1, 1, 1, 1, 1, 1);

  subhook_t socket_hook =
      subhook_new(reinterpret_cast<void*>(socket),
  ...
  subhook_t setsockopt_hook = subhook_new(
...
#ifdef WIN_OS
  subhook_t ioctlsocket_hook = subhook_new(
      reinterpret_cast<void*>(ioctlsocket),
...
#else
  subhook_t fcntl_hook =
      subhook_new(reinterpret_cast<void*>(fcntl),
  ...
  subhook_t select_m1_hook = subhook_new(
      reinterpret_cast<void*>(select), reinterpret_cast<void*>(fail_select_m1),
  ...
  subhook_t select_0_hook =
      subhook_new(reinterpret_cast<void*>(select),
  ...
  subhook_t getsockopt_1_hook = subhook_new(
  ...
  subhook_t select_1_hook = subhook_new(reinterpret_cast<void*>(select),
                                        reinterpret_cast<void*>(success_select),
                                        SUBHOOK_64BIT_OFFSET);
  ...
  subhook_install(select_1_hook);
  subhook_t getsockopt_0_hook = subhook_new(
      reinterpret_cast<void*>(getsockopt),
...
TEST(uni_io_manager, read_inputs) {
  ...
  mngr.init(4);
  mngr.add_node(0, io_manager::io_node::TYPES::PHOENIX_BK_ETH, 1, "127.0.0.1",
                "A100", 1, 1, 1, 1, 1, 1);
  mngr.add_node(1, io_manager::io_node::TYPES::PHOENIX_BK_ETH, 2, "127.0.0.1",
                "A200", 1, 1, 1, 1, 1, 1);
  mngr.get_node(1)->is_active = false;
  mngr.add_node(2, io_manager::io_node::TYPES::WAGO_750_XXX_ETHERNET, 3,
                "127.0.0.1", "A300", 1, 1, 1, 1, 1, 1);
  mngr.add_node(3, io_manager::io_node::TYPES::WAGO_750_XXX_ETHERNET, 4,
                "127.0.0.1", "A400", 1, 1, 1, 1, 1, 1);
