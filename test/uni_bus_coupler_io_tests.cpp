...
TEST( io_node, status_register_only_for_phoenix )
    ...
    EXPECT_EQ( phoenix_node->get_display_state(), io_manager::io_node::DST_WARNING );
    EXPECT_EQ( wago_node->get_display_state(), io_manager::io_node::DST_OK );
...
TEST( io_node, error_bits_detection )
    ...
    EXPECT_EQ( node->get_display_state(), io_manager::io_node::DST_OK );
    
    // Test each individual error bit (0-5).
    node->status_register = 0x0001;  // Bit 0: Error occurred.
    EXPECT_EQ( node->get_display_state(), io_manager::io_node::DST_WARNING );
...
TEST( uni_io_manager, read_phoenix_status_register_byte_order_conversion )
    ...
    EXPECT_EQ( node->get_display_state(),
        io_manager::io_node::DST_WARNING );
